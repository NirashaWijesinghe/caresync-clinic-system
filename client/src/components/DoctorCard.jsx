...
export default function DoctorCard({ doctor, onBookNow }) {
  return (
    <div className="bg-white rounded-3xl border border-slate-200/90 overflow-hidden shadow-2xs hover:shadow-xl hover:border-blue-300 hover:-translate-y-1 transition-all duration-300 flex flex-col group">
      <div className="p-6 flex-1 flex flex-col">
        ...
        <div className="flex gap-4 items-start">
          <div className="relative">
            ...
            />
            <span className="absolute -bottom-1 -right-1 bg-emerald-500 w-4 h-4 rounded-full border-2 border-white shadow-xs" title="Verified Consultant"></span>
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2.5 mt-2 text-xs text-slate-600">
              <span className="flex items-center text-amber-600 font-bold gap-1 bg-amber-50 px-2 py-0.5 rounded-md border border-amber-100/60">
      ...
      <div className="px-6 py-4 bg-slate-50/80 border-t border-slate-100 flex items-center justify-between">
        <div>
          <span className="text-[10px] text-slate-400 block font-semibold uppercase tracking-wider">Fee</span>
        ...
        <div className="flex items-center gap-2">
          <Link
            ...
            className="text-xs font-bold text-slate-600 hover:text-blue-600 px-3 py-2 rounded-xl hover:bg-white transition-all"
          >
            Profile
          </Link>
          <button
            onClick={() => onBookNow ? onBookNow(doctor) : null}
            className="text-xs font-bold text-white bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded-xl shadow-xs shadow-blue-500/20 flex items-center gap-1.5 transition-all group-hover:shadow-md"
          >
            Book Slot
