...
import { Search, Stethoscope, Filter, X, Sparkles, CheckCircle2 } from "lucide-react";
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14 space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 pb-2 border-b border-slate-200/80">
        ...
        <div className="text-xs font-semibold text-slate-600 bg-white px-4 py-2 rounded-xl border border-slate-200 shadow-2xs">
          Showing <span className="text-blue-600 font-bold">{doctors.length}</span> Active Specialists
        </div>
      </div>

      {/* Filter & Search Bar */}
      <div className="bg-white p-4 sm:p-5 rounded-3xl border border-slate-200/90 shadow-sm space-y-4">
        {/* Top: Search Input & Clear */}
        <form onSubmit={handleSearchSubmit} className="flex flex-col sm:flex-row gap-3 items-center">
          <div className="relative flex-1 w-full">
            <Search className="w-4 h-4 text-slate-400 absolute left-4 top-3.5" />
            <input
              type="text"
              placeholder="Search by doctor name, hospital, or clinical specialty..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full pl-11 pr-4 py-3 rounded-2xl border border-slate-200 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
            />
            {searchTerm && (
              <button
                type="button"
          ...
          <button
            type="submit"
            className="w-full sm:w-auto px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold rounded-2xl shadow-sm shadow-blue-500/20 transition-all flex items-center justify-center gap-2"
          >
            Search Panel
          </button>
          {(selectedSpecialty || searchTerm) && (
            <button
              ...
              className="w-full sm:w-auto px-4 py-3 text-slate-500 hover:text-slate-800 text-xs font-semibold rounded-2xl border border-slate-200 hover:bg-slate-50 transition-all"
            >
              Reset Filters
            </button>
          )}
      ...
      {loading ? (
        ...
        <div className="p-16 text-center bg-white rounded-3xl border border-slate-200/90 shadow-sm space-y-4 max-w-md mx-auto">
